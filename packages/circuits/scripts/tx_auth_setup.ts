...
async function execTxAuth() {
  ...
  const txAuthPathR1csPath = path.join(buildDir, "tx_auth/tx_auth.r1cs");
  if (!fs.existsSync(txAuthPathR1csPath)) {
    throw new Error(`${txAuthPathR1csPath} does not exist.`);
  }
  await generateKeys(
    phase1Path,
    txAuthPathR1csPath,
